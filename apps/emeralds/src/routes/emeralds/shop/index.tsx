...
export const Route = createFileRoute("/emeralds/shop/")({
	head: ({ loaderData: _ld }) => {
		const loaderData = _ld as { emeralds: EmeraldWithImage[] } | undefined;
		const emeralds = loaderData?.emeralds ?? [];
		return buildMeta({
	...
	loader: async ({ context }): Promise<{ emeralds: EmeraldWithImage[] }> => {
	...
	const filteredProducts = useMemo(() => {
		return emeralds.filter((product) => {
			...
			if (
				...
				return false;
			if (filters.cuts.length > 0 && !filters.cuts.includes(product.cut as Cut))
				return false;
			return true;
		});
	}, [emeralds, filters]);
