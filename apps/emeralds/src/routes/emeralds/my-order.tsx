...
import {
	...
	orderLookupSchema,
} from "@/lib/schemas/order-lookup.schema";
...
function MiPedidoPage() {
	...
	const mutation = useMutation({
		mutationFn: async (values: OrderLookupValues) => {
	...
	const form = useForm({
		defaultValues: { order_number: "", contact: "" } as OrderLookupValues,
							...
							<>
								<div className="text-center space-y-1">
									...
									<p className="text-sm text-brand-primary-dark/55">
										Ingresa tu número de pedido y el email o WhatsApp que usaste
										al hacer la compra.
									</p>
								</div>
								...
								>
									<form.Field
										name="order_number"
										validators={{
											onBlur: orderLookupSchema.shape.order_number,
											onSubmit: orderLookupSchema.shape.order_number,
										}}
									>
										{(field) => (
									...
									<form.Field
										name="contact"
										validators={{
											onBlur: orderLookupSchema.shape.contact,
											onSubmit: orderLookupSchema.shape.contact,
										}}
									>
										{(field) => (
											<div className="space-y-1.5">
